...
svip=$(ifconfig | grep 'inet' | awk -F'inet' '{ print $2}' | awk '{ print $1}' | head -1)
...
rpm -Uvh http://rpms.famillecollet.com/enterprise/7/remi/x86_64/remi-release-7.3-1.el7.remi.noarch.rpm
...
cp $script_path/bin/pineauto  /bin/pineauto && chmod +x /bin/pineauto
cp -rf $script_path/component/* /etc/pineauto/menu/ && chmod +x -R /etc/pineauto/menu/
